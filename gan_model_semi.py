...
class AdverGAN(keras.Model):
  ...
  def compile(self, g_opt, d_opt, gan_loss, gan_metric, disc_metric_fn,
              g_loss_fn, g_metric_fn, semi_loss, g_tracker, gan_tracker, disc_tracker):
  ...
  def train_step(self, data):
    ...
    if self.lambda_semi > 0.0:
      imgs, masks, semi_imgs = data
    else:
      imgs, masks = data

    # Generate masks
    generated_masks = self.gen(imgs)
    # Generate masks for semi learning and confidence for them
    if self.lambda_semi > 0.0:
      generated_semi_masks = self.gen(semi_imgs)
      semi_confidence = self.disc(generated_semi_masks)
    ...
    with tf.GradientTape() as tape:
      ...
      if self.lambda_semi > 0.0:
        semi_loss = tf.reduce_mean(self.semi_loss(generated_semi_masks, semi_confidence, self.threshold_semi, self.n_class))
        total_g_loss = g_loss + self.lambda_adv * gan_loss + self.lambda_semi * semi_loss
  ...
  def save_models(self, models_save_dir):
    self.gen.save(os.path.join(models_save_dir, "generator_semi.h5"))
    self.disc.save(os.path.join(models_save_dir, "discriminator_semi.h5"))
